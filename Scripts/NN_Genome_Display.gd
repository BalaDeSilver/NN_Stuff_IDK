...
# Look, I know my code sucks, I'll improve later, okay?
...
func redraw():
	...
	var big_layer = 0
	for i in range(nodes_by_layer.size()):
		if(nodes_by_layer[i].size() > big_layer):
			big_layer = nodes_by_layer[i].size()
		nodes_by_layer[i].append((nodes_by_layer[i].size() * General_Manager.node_size.y + nodes_by_layer[i].size() * General_Manager.node_spacing.y) / 2)
	var middle = (big_layer * General_Manager.node_size.y + big_layer * General_Manager.node_spacing.y) / 2
	
	for i in range(nodes_by_layer.size()):
		for j in range(nodes_by_layer[i].size() - 1):
			nodes_by_layer[i][j].position = Vector2((i * General_Manager.node_size.x + i * General_Manager.node_spacing.x), ((j * General_Manager.node_size.y + j * General_Manager.node_spacing.y) + (middle - nodes_by_layer[i].back())))
...
func redefine_margin(agent_id):
	if(agent_id == ref_genome.agent_ref.id):
		...
		for i in range(ref_genome.nodes.size()):
			if(ref_genome.nodes[i].canvas_location.x + General_Manager.node_size.x > temp_br.x):
				temp_br.x = ref_genome.nodes[i].canvas_location.x + General_Manager.node_size.x
			if(ref_genome.nodes[i].canvas_location.x < temp_tl.x):
				temp_tl.x = ref_genome.nodes[i].canvas_location.x
			if(ref_genome.nodes[i].canvas_location.y + General_Manager.node_size.y > temp_br.y):
				temp_br.y = ref_genome.nodes[i].canvas_location.y + General_Manager.node_size.y
